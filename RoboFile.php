...
//  }
  ...
  public function prepareSourcecode($opts = [
    ...
    $this->taskExecStack()
      ->dir('src/DevShop/Components/DevShopControlTemplate')
  ...
  public function up($docker_command = '', $opts = [
    ...
    if ($opts['mode'] == 'docker-compose') {
      ...
      if (empty($_SERVER['CI'])) {
        ...
        $extra_vars['devshop_control_path'] = '/usr/share/devshop/src/DevShop/Components/DevShopControlTemplate';
...
//    }
  ...
  public function destroy($opts = ['force' => 0]) {
    if ($opts['no-interaction'] || $this->confirm("Destroy all local data? (docker containers, volumes, config)")) {
      ...
      $this->_exec("sudo rm -rf src/DevShop/Components/DevShopControlTemplate/web/sites/{$uri}");
    }
