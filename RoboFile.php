  ...
  public function up($opts = [
    ...
    if ($opts['mode'] == 'docker-compose') {
      ...
      elseif ($opts['test-upgrade']) {
        ...
        $cmd[] = "docker-compose -f {$this->devshop_root_path}/docker-compose-tests.yml --name devshop_server --detached run $env -e UPGRADE_FROM_VERSION={$version} -e AEGIR_HOSTMASTER_ROOT=/var/aegir/devmaster-{$version} -e AEGIR_HOSTMASTER_ROOT_TARGET=$root_target -e AEGIR_VERSION={$version} -e AEGIR_MAKEFILE=https://raw.githubusercontent.com/opendevshop/devshop/{$version}/build-devmaster.make -e PROVISION_VERSION={$provision_version} devshop";
        $cmd[] = "docker exec devshop_server '$command'";
      }
      else {
