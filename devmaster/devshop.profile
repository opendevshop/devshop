...
function devshop_profile_modules() {
  return array(
    ...
    /* other contrib */ 'install_profile_api' /* needs >= 2.1 */, 'jquery_ui', 'jquery_update', 'modalframe', 'admin_menu',
    ...
    /* NICEITIES */
    'hosting_drush_aliases', 'hosting_filemanager', 'hosting_logs', 'hosting_queue_runner', 'hosting_solr', 'hosting_tasks_extra',
  );
}
 ...
 */
function devshop_profile_details() {
  return array(
    'name' => 'DevShop',
 ...
 */
function devshop_profile_tasks(&$task, $url) {
  // Install dependencies
  install_include(devshop_profile_modules());
  ...
  devshop_task_finalize();
}

function devshop_bootstrap() {
  /* Default node types and default node */
  $types =  node_types_rebuild();

  variable_set('install_profile', 'devshop');
