...
docker run --rm --name devshop-server --hostname devshop.server.local.computer -t -i -e MYSQL_ROOT_PASSWORD=securepass -v devshop-projects:/var/aegir/projects -p 80:80 -d devshop/server:local
